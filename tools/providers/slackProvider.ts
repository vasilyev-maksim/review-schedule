...
export const slackProvider: IProvider = {
    async findReviewers (names: string[]): Promise<Partial<IReviewer>[]> {
        return axios.get(url).then(({ data }: AxiosResponse<IResponse>) => {
            if (data.ok) {
                ...
                if (data.error === 'token_revoked') {
                    ...
                    throw Error(`Unknown error from Slack API: ${data.error}`);
                }
            }
        });
    }
